# Privacy Policy for Mumbleflux Arcade

**Effective date:** 17 June 2026
**Application name:** MumbleFlux Arcade (formerly Fluxrider)
**Developer / publisher:** MumbleFlux 
**Developer type:** Individual, non-corporate developer  
**Contact email:** [mumbleflux@outlook.com](mailto:mumbleflux@outlook.com)

This Privacy Policy explains how information is handled when you use **Mumbleflux Arcade**, an Android mobile application published by **MumbleFlux**.

Mumbleflux Arcade is distributed through the **Google Play Store** and is **not intended for use by people under the age of 13**.

## 1. Overview

MumbleFlux does not operate user accounts and does not directly collect names, email addresses, phone numbers, payment information, contact lists, photos, precise location information, or other similar personal information through Mumbleflux Arcade.

Mumbleflux Arcade uses **Google AdMob** to display interstitial advertisements. Any personal data, device data, advertising data, analytics data, diagnostic data, or similar information collected for advertising-related purposes is collected and processed by **Google AdMob**, the **Google Mobile Ads SDK**, Google, and, where applicable, Google's advertising partners.

MumbleFlux may receive aggregated or non-identifying advertising reports through Google AdMob, such as ad performance metrics, but does not receive user-level personal information directly from Mumbleflux Arcade.

## 2. Information collected by Mumbleflux Arcade

MumbleFlux does not directly collect personal information from users through Mumbleflux Arcade.

Mumbleflux Arcade may store basic app preferences or gameplay-related information locally on your device, such as settings, scores, or progress, if required for app functionality. This locally stored information is not transmitted to MumbleFlux unless you separately choose to provide information to MumbleFlux, for example by contacting support by email.

## 3. Information collected by Google AdMob

Because Mumbleflux Arcade uses Google AdMob, Google may automatically collect and process certain information when advertisements are requested, loaded, displayed, or interacted with.

Depending on your device, region, consent choices, Google account settings, Android settings, and Google's own policies, this may include:

- IP address, which may be used to estimate general location;
- device identifiers, including the Android Advertising ID, app set ID, and similar identifiers;
- app interaction information, such as app launches, taps, ad views, and video views;
- advertising interaction information;
- diagnostic information, such as SDK performance, app launch time, hang rate, crash-related signals, and energy usage;
- information used for advertising, ad measurement, analytics, security, fraud prevention, and abuse prevention.

Google's AdMob and Google Mobile Ads SDK disclosures are available here:

- [Google Mobile Ads SDK — Play Data Disclosure](https://developers.google.com/admob/android/privacy/play-data-disclosure)
- [Google Privacy Policy](https://policies.google.com/privacy)
- [How Google uses information from sites or apps that use Google services](https://policies.google.com/technologies/partner-sites)
- [Google Advertising Technologies](https://policies.google.com/technologies/ads)

## 4. How information is used

Information collected through Google AdMob may be used by Google and its advertising partners for purposes including:

- serving advertisements in Mumbleflux Arcade;
- displaying personalised or non-personalised advertisements, depending on your settings, consent choices, and applicable law;
- measuring ad performance;
- limiting fraud, invalid traffic, abuse, and security risks;
- improving advertising systems and related analytics;
- complying with legal and regulatory obligations.

MumbleFlux uses AdMob to support the availability of Mumbleflux Arcade by displaying advertisements.

## 5. Third-party services

Mumbleflux Arcade uses the following third-party service:

### Google AdMob

Google AdMob is an advertising service provided by Google. Google may collect, receive, use, disclose, and share information as described in Google's own privacy policies, advertising policies, SDK documentation, and applicable terms.

MumbleFlux does not control all processing performed by Google, Google AdMob, the Google Mobile Ads SDK, or Google's advertising partners.

You can learn more about Google's privacy practices here:

- [Google Privacy Policy](https://policies.google.com/privacy)
- [Google Advertising Technologies](https://policies.google.com/technologies/ads)
- [Google Ad Settings](https://adssettings.google.com/)

## 6. Personalised advertising and consent

Depending on your location and applicable law, Google AdMob may request, rely on, or process consent for personalised advertising, non-personalised advertising, ad measurement, local storage, or related advertising purposes.

Where required, users may be presented with consent options in the app or through Google-controlled mechanisms. Users may also be able to manage advertising preferences through Android device settings, Google account settings, and Google Ad Settings.

For users in the European Economic Area, the United Kingdom, and Switzerland, Google's EU User Consent Policy may apply to the use of cookies, local storage, and personal data for advertising purposes:

- [Google EU User Consent Policy](https://www.google.com/about/company/user-consent-policy/)

## 7. Advertising ID and device controls

Android users may be able to reset or delete their Advertising ID through their device settings. The exact setting path may vary depending on Android version and device manufacturer, but it is commonly available under privacy or ads settings.

You may also be able to adjust Google advertising settings here:

- [Google Ad Settings](https://adssettings.google.com/)

## 8. Data sharing

MumbleFlux does not sell user personal information directly.

Information collected through Google AdMob may be collected, used, disclosed, shared, or processed by Google, Google AdMob, the Google Mobile Ads SDK, advertising technology providers, and advertising partners for advertising, measurement, analytics, diagnostics, fraud prevention, security, and related purposes.

Where privacy laws define certain advertising activities as a "sale", "sharing", or use for "cross-context behavioural advertising", Google AdMob's advertising-related processing may fall within those definitions depending on the user's location, settings, consent choices, and applicable law.

## 9. Data retention and deletion

**MumbleFlux does not maintain user accounts for Mumbleflux Arcade and does not maintain a server-side database of Mumbleflux Arcade users**.

Any advertising-related data collected by Google AdMob is retained, managed, and deleted according to Google's policies and user controls.

If you contact MumbleFlux by email, MumbleFlux may retain your email address and the contents of your message as reasonably necessary to respond to your enquiry, provide support, manage privacy requests, maintain business records, resolve disputes, or comply with legal obligations.

To request deletion of information that you have directly provided to MumbleFlux by email, contact:

[mumbleflux@outlook.com](mailto:mumbleflux@outlook.com)

Because MumbleFlux does not operate user accounts or directly collect app user identifiers, MumbleFlux may not be able to identify or delete information collected by Google AdMob. Requests about Google-controlled data may need to be made directly through Google's privacy tools or support channels.

## 10. Children's privacy

Mumbleflux Arcade is not intended for use by people under the age of 13.

MumbleFlux does not knowingly collect personal information from children under 13. If you believe that a child under 13 has provided personal information directly to MumbleFlux, please contact:

[mumbleflux@outlook.com](mailto:mumbleflux@outlook.com)

MumbleFlux will take reasonable steps to delete that information.

If Mumbleflux Arcade is used by a child despite not being intended for children, Google AdMob may still process information depending on device settings, Google account settings, applicable laws, and ad-serving configuration.

## 11. GDPR, UK GDPR, and EEA/UK/Swiss privacy rights

This section applies to users located in the European Economic Area, the United Kingdom, or Switzerland, where the GDPR, UK GDPR, or similar data protection laws apply.

### Controller

For personal information that you provide directly to MumbleFlux, such as support or privacy emails sent to [mumbleflux@outlook.com](mailto:mumbleflux@outlook.com), the relevant controller is:

**MumbleFlux**  
Email: [mumbleflux@outlook.com](mailto:mumbleflux@outlook.com)

For advertising-related information collected through Google AdMob, Google may act as a controller, independent controller, joint controller, or processor depending on the specific processing activity and Google's applicable terms.

### Legal bases

Depending on the type of processing and applicable law, the legal bases for processing may include:

- **Consent**, including for personalised advertising, local storage, and related advertising purposes where consent is required;
- **Legitimate interests**, including app operation, non-personalised advertising, fraud prevention, diagnostics, security, and responding to user enquiries;
- **Legal obligations**, where processing is necessary to comply with applicable laws;
- **Performance of a request**, where you contact MumbleFlux for support, privacy enquiries, or related assistance.

### GDPR rights

Subject to applicable law and limitations, you may have the right to:

- request access to your personal data;
- request correction of inaccurate or incomplete personal data;
- request deletion of your personal data;
- request restriction of processing;
- object to certain processing;
- request data portability;
- withdraw consent where processing is based on consent;
- lodge a complaint with a relevant data protection authority.

To exercise rights relating to information you have provided directly to MumbleFlux, contact:

[mumbleflux@outlook.com](mailto:mumbleflux@outlook.com)

For data processed by Google AdMob, you may also need to use Google's own privacy controls or contact Google directly.

## 12. CCPA and CPRA notice for California residents

This section applies to California residents where the California Consumer Privacy Act, as amended by the California Privacy Rights Act, applies.

### Categories of personal information

MumbleFlux does not directly collect personal information through Mumbleflux Arcade.

Through Google AdMob, the following categories of personal information may be collected or processed by Google, depending on the user, device, settings, consent choices, and applicable law:

| Category | Examples |
| --- | --- |
| Identifiers | IP address, Android Advertising ID, app set ID, device identifiers |
| Internet or electronic network activity | App launches, taps, ad views, ad interactions, video views |
| Approximate location | General location estimated from IP address |
| Diagnostics and app performance data | SDK diagnostics, app launch time, hang rate, energy usage, crash-related signals |
| Inferences | Advertising-related interests or ad personalisation signals, where applicable |

### Purposes for collection or processing

These categories may be used for:

- advertising;
- personalised or non-personalised ad delivery;
- ad measurement and analytics;
- fraud prevention and security;
- diagnostics and performance measurement;
- compliance with legal obligations.

### Sale or sharing of personal information

MumbleFlux does not directly sell personal information for money.

However, the use of third-party advertising SDKs may be considered a "sale" or "sharing" of personal information under some privacy laws where data is used for cross-context behavioural advertising or personalised advertising.

California residents may opt out of certain personalised advertising or limit ad tracking through Google and Android advertising settings. You may also contact MumbleFlux at [mumbleflux@outlook.com](mailto:mumbleflux@outlook.com) with privacy requests.

### Sensitive personal information

MumbleFlux does not knowingly collect sensitive personal information through Mumbleflux Arcade.

### Financial incentives

MumbleFlux does not offer financial incentives in exchange for the collection, sale, or sharing of personal information.

### California privacy rights

Subject to applicable law and verification, California residents may have the right to:

- know what personal information is collected, used, disclosed, sold, or shared;
- request access to personal information;
- request correction of inaccurate personal information;
- request deletion of personal information;
- opt out of the sale or sharing of personal information;
- limit the use or disclosure of sensitive personal information, where applicable;
- not be discriminated against for exercising privacy rights.

To make a California privacy request, contact:

[mumbleflux@outlook.com](mailto:mumbleflux@outlook.com)

MumbleFlux may need to verify your request before responding. Because MumbleFlux does not operate user accounts or directly collect app user identifiers, MumbleFlux may not be able to identify information collected by Google AdMob. For advertising data processed by Google, you may also need to use Google's own privacy controls or contact Google directly.

## 13. CalOPPA notice for California users

This section is provided for purposes of the California Online Privacy Protection Act.

### Categories of personally identifiable information collected

MumbleFlux does not directly collect personally identifiable information through Mumbleflux Arcade.

Google AdMob may collect advertising-related identifiers, device information, IP address, app interaction information, ad interaction information, and diagnostic information as described in this Privacy Policy.

### Third-party collection

Google AdMob and Google's advertising partners may collect information about your use of Mumbleflux Arcade and other apps or services over time for advertising, ad measurement, analytics, diagnostics, fraud prevention, and security purposes.

### Do Not Track

Mumbleflux Arcade is a mobile application and does not respond to web browser "Do Not Track" signals.

You can control certain advertising and tracking preferences through Android device settings, Google account settings, Google Ad Settings, and any consent or privacy options made available in the app.

### Changes to this Privacy Policy

MumbleFlux may update this Privacy Policy from time to time. Material changes will be reflected by updating the effective date above and, where appropriate, by providing notice in the app or through the app listing.

## 14. International data transfers

Google AdMob and related Google services may process information on servers located outside your state, province, or country. As a result, information may be processed in countries with privacy laws that differ from those in your location.

## 15. Security

MumbleFlux does not maintain a user account database for Mumbleflux Arcade.

Google states that data collected by the Google Mobile Ads SDK is encrypted in transit. Advertising-related information is handled according to Google's own security and data handling practices.

## 16. Changes to this Privacy Policy

MumbleFlux may update this Privacy Policy from time to time to reflect changes to Mumbleflux Arcade, Google AdMob, legal requirements, or developer practices.

When changes are made, the effective date at the top of this Privacy Policy will be updated. Continued use of Mumbleflux Arcade after a Privacy Policy update means you accept the updated Privacy Policy.

## 17. Contact

For privacy questions, requests, or concerns, contact:

**MumbleFlux**  
Email: [mumbleflux@outlook.com](mailto:mumbleflux@outlook.com)
